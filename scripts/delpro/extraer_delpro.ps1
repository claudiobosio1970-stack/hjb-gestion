...
# 3. MODO SIMULACION / PRUEBA LOCAL (SI NO HAY SQL SERVER EN ESTA MAQUINA)
if ($Simular) {
    Write-Host ""
    ...
    [System.IO.File]::WriteAllText((Resolve-Path -Path "." -ErrorAction SilentlyContinue).Path + "\" + $RutaSalida.TrimStart(".\"), $json, $utf8NoBom)
    Write-Host "Archivo generado exitosamente en: $RutaSalida" -ForegroundColor Green
    return
}

# 4. Conexion Real a SQL Server
Write-Host ""
...
# 5. Extraccion de Metricas de Ordeñe
...
if ($dtProd -and $dtProd.Rows.Count -gt 0 -and $dtProd.Rows[0]["VacasEnOrdeñe"] -ne [DBNull]::Value -and [int]$dtProd.Rows[0]["VacasEnOrdeñe"] -gt 0) {
    $vacasVO = [int]$dtProd.Rows[0]["VacasEnOrdeñe"]
...
# 6. Extraccion de Vacas Secas / Preparto
$sqlPreparto = @"
SELECT COUNT(DISTINCT a.OID) AS VacasSecas
FROM Animal a WITH (NOLOCK)
WHERE a.LactationStatus IN (0, 2) AND a.Pregnant = 1;
"@
...
# 7. Extraccion de Partos Recientes con Segregacion HJB
...
SELECT TOP 20
    ...
    ISNULL(k.VisualID, 'Sin RP') AS RPCria,
...
# 8. Compilar Objeto Final y Guardar JSON
...
$utf8NoBom = New-Object System.Text.UTF8Encoding($false)
$rutaCompleta = (Resolve-Path -Path "." -ErrorAction SilentlyContinue).Path + "\" + $RutaSalida.TrimStart(".\")
...
Write-Host "Podes cargar este archivo directamente en la solapa DelPro de HJB Gestion."
